...
export class CasoTestesCadastro {
  conferirPagina() {
    cy.url(TEXTO_CADASTRO.PAGINA_CADASTRO);
    cy.contains(LOC_CADASTRO.TITULO, TEXTO_CADASTRO.PARTE_TITULO);
  }
  ...
  preencherCepIncorreto() {
    cy.get(LOC_CADASTRO.CAMPO_CEP).type(INSERIR_TEXTO.CAMPO_CEP_INCORRETO);
  }

  clicarBotaoVoltarHome() {
    cy.get(LOC_CADASTRO.BOTAO_VOLTAR_HOME).click();
  }

  preencherCpfIncorreto() {
    cy.get(LOC_CADASTRO.CAMPO_NOME_COMPLETO).type(INSERIR_TEXTO.CAMPO_NOME);
  ...
  checandoValores() {
    ...
    cy.get(LOC_CADASTRO.CAMPO_CPF).should(
      ASSERTIVA.COMPARA_VALOR,
      INSERIR_TEXTO.CAMPO_CPF_CORRETO
    );
  ...
  selecionarMoto() {
    cy.get(LOC_CADASTRO.BOTAO_MOTO).click();
  }

  selecionarBicicleta() {
    cy.get(LOC_CADASTRO.BOTAO_BICICLETA).click();
  }

  selecionarCarro() {
    cy.get(LOC_CADASTRO.BOTAO_CARRO).click();
  }
  ...
  confereMsgCepInvalido() {
    cy.get(LOC_ERRO.CEP_INCORRETO).should(
      ASSERTIVA.COMPARA_TEXTO,
      TEXTO_ERRO.CEP_INCORRETO
    );
  }

  confereMsgCpfInvalido() {
    cy.get(LOC_ERRO.CPF_INVALIDO).should(
      ASSERTIVA.COMPARA_TEXTO,
      TEXTO_ERRO.CAMPO_CPF_INVALIDO
    );
  }

  erroMetodosEntrega() {
    cy.get(LOC_ERRO.VARIOS_METODOS_ENTRADA).should(
      ASSERTIVA.COMPARA_TEXTO,
      TEXTO_ERRO.VARIOS_METODOS_ENTREGA
    );
  }
...
const casoTestesCadastro = new CasoTestesCadastro();
export default casoTestesCadastro;
